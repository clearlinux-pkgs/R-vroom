...
Source0  : https://cran.r-project.org/src/contrib/vroom_1.5.7.tar.gz
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1641147548
...
R CMD INSTALL --install-tests --built-timestamp=${SOURCE_DATE_EPOCH} --build  -l %{buildroot}/usr/lib64/R/library vroom
for i in `find %{buildroot}/usr/lib64/R/ -name "*.so"`; do mv $i $i.avx2 ; mv $i.avx2 ~/.stash/; done
echo "CFLAGS = $CFLAGS -march=x86-64-v4 -ftree-vectorize  -mno-vzeroupper " > ~/.R/Makevars
echo "FFLAGS = $FFLAGS -march=x86-64-v4 -ftree-vectorize  -mno-vzeroupper " >> ~/.R/Makevars
echo "CXXFLAGS = $CXXFLAGS -march=x86-64-v4 -ftree-vectorize -mno-vzeroupper  " >> ~/.R/Makevars
R CMD INSTALL --preclean --install-tests --no-test-load --built-timestamp=${SOURCE_DATE_EPOCH} --build  -l %{buildroot}/usr/lib64/R/library vroom
for i in `find %{buildroot}/usr/lib64/R/ -name "*.so"`; do mv $i $i.avx512 ; mv $i.avx512 ~/.stash/; done
echo "CFLAGS = $CFLAGS -ftree-vectorize " > ~/.R/Makevars
echo "FFLAGS = $FFLAGS -ftree-vectorize " >> ~/.R/Makevars
echo "CXXFLAGS = $CXXFLAGS -ftree-vectorize " >> ~/.R/Makevars
R CMD INSTALL --preclean --install-tests --built-timestamp=${SOURCE_DATE_EPOCH} --build  -l %{buildroot}/usr/lib64/R/library vroom
...
R CMD check --no-manual --no-examples --no-codoc vroom || :
...
/usr/lib64/R/library/vroom/tests/testthat/logs/fixed_width_index.idx
/usr/lib64/R/library/vroom/tests/testthat/logs/fixed_width_index_connection.idx
/usr/lib64/R/library/vroom/tests/testthat/logs/index.idx
/usr/lib64/R/library/vroom/tests/testthat/logs/index_connection.idx
/usr/lib64/R/library/vroom/tests/testthat/mtcars.zip
